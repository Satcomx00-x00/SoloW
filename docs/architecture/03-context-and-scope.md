...
SoloW sits between the people who direct harness work and the external tools and
environments that work happens in and against.

**People (actors):**
- **User** — creates Issues and Tasks, runs harnesses, designs Workflows, reviews changes.
...
- **AI coding harnesses** — external command-line harness tools that SoloW drives to do the
  work.
...
- **Execution environments** — the machines and containers harnesses run in (local host,
  containers, remote hosts, cloud runners).
- **Model providers** — the AI services the harnesses themselves call, either on a user's
...
> **SoloW** (the system)
>   ↔ drives → AI coding harnesses
>   ↔ isolates work in → Git repositories, integrates via → source hosts
>   ↔ synchronises → issue trackers
>   ↔ notifies via → chat
>   ↔ runs harnesses in → execution environments
>   ↔ harnesses call → model providers (subscription or API key)

## In scope for SoloW

- Orchestrating harnesses, isolating their work, and supervising their runs.
...
- **Toward harnesses:** a single standard protocol boundary (see [Decision 0003](../decisions/0003-agent-connection-protocol.md)).
- **Toward source hosts and trackers:** integration connectors, all optional
  (see [F12](../features/F12-integrations.md)).
- **Toward model providers:** credentials are supplied to harnesses without exposing them to
  harness-run code (see [F17](../features/F17-security-secrets.md)).
