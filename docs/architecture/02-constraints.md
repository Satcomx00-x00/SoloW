...
- **C-4 Billing integrity.** A Subscription-mode harness must never be run in a way that causes
  metered billing.
- **C-5 Credential isolation.** Harness-run code must never have access to raw credentials.

## Technical constraints

- **C-6 Standard harness protocol.** Harnesses are integrated through a single open standard
  (the Agent Client Protocol) rather than bespoke per-harness integrations
  (see [Decision 0003](../decisions/0003-agent-connection-protocol.md)).
- **C-7 Long-lived harness processes.** Harnesses are external, long-running command-line
...
- **C-8 Isolation via working copies.** Parallel harness work is isolated using per-Task Git
  working copies.
...
(C-8), a standard protocol boundary for harnesses (C-6), and a portable, credential-safe
