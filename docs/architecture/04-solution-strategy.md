...
Harnesses are long-running external processes that must be held open, supervised, and streamed
...
data) and a **long-lived orchestration component** (launches harnesses, holds their processes,
...
## Strategy 2 — Integrate harnesses through one standard protocol

Rather than integrating each harness tool bespoke, SoloW connects to all of them through
a single open standard (the Agent Client Protocol). Adding a new harness is a configuration
...
concurrent harnesses cannot collide. Multi-repository Tasks are a natural extension of this.
→ [F08](../features/F08-workspaces-repositories.md)

## Strategy 5 — Portable, credential-safe billing modes

Billing mode is a property of a Harness Profile. Subscription mode uses a portable credential
that works across all execution environments; the orchestrator guarantees a subscription
harness is never run in a way that causes metered billing, and never exposes credentials to
harness-run code. → [Decision 0005](../decisions/0005-subscription-authentication.md)
...
multi-harness processes understandable at a glance and steerable in real time.
...
work; the standard protocol keeps harnesses interchangeable; per-Task isolation keeps
