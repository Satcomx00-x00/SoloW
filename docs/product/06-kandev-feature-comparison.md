...
| **GateControl today** | What exists in this repository, with the code that backs the claim |
| **Status** | ✅ built · 🟡 partial · 📄 spec only · ❌ absent · ⭐ GateControl-only |
...
| 13 | Multi-branch tasks (several PRs) | 1 branch per task (`gatecontrol/task-<id>`) | ❌ | Falls out of 12 **for free** if the join key is `(repository, branch)` rather than repository. Choosing that key now, while still single-repo, is the cheapest future-proofing decision on this list | "Add branch" row inside the repo group: `existing` Input + Select for the base ref |
...
| # | kandev capability | GateControl today | Status | Best implementation — and what it unlocks | UI shape & components |
|---|---|---|---|---|---|
| 19 | 21+ agents (Codex, Copilot, Gemini, Cursor, Devin, Qwen…) | `agentKindSchema = ["claude_code"]` | 🟡 | Kill the enum. Agent identity becomes an **`agent_catalog` row** — command, args template, protocol, declared capabilities, install/version probe — so adding an agent is a seed row plus a profile, not a code change. → this is what turns "multi-provider" from engineering into configuration, and it gates 20, 21, 26, 27, 60 | Settings → Agents: a grid of `existing` Card tiles with install state and version. The agent picker becomes a **new** Combobox with icon + capability `existing` Badges |
...
| 45 | Git worktree isolation | `worktree` table + provision/adopt/cleanup, and Claude Code's own `--worktree` for the run | ✅ | **Unify ownership on GateControl.** Isolation currently leans partly on the agent's own worktree flag, which means every new agent (19) must re-implement isolation or it silently regresses — the one guarantee with zero tolerance in the constitution | Worktree path and branch as a copyable row in the task header (`existing` Tooltip + copy Button) |
...
| 50 | Executor profile config (prepare scripts, env, credentials) | Record with name + kind only, no config payload | 🟡 | Give the profile a **typed `config` JSON validated by a per-kind discriminated Zod union** — one table, N shapes, no migration per executor kind. This single decision is what makes 46–48 additive instead of schema-breaking | A form **rendered dynamically from the selected kind**: `existing` Select → conditional `existing` Form fields, monospace prepare-script Textarea, env key/value repeater rows |
...
| 60 | Subscription vs API-key billing | Subscription runs strip `ANTHROPIC_API_KEY` from the child env | ✅ ⭐ | Move the strip rule **onto the agent catalog row (19)** — each agent declares its subscription variable and its metered variable — so the guarantee holds as agents are added. Hardcoded to one agent, this differentiator dies the moment agent #2 lands | Auth mode as a **new** RadioGroup on the profile with a per-mode explanation, and a warning `existing` Badge whenever a profile could bill metered |
...
| 70 | Jira | `F12` spec only | 📄 | Driver over 68, with field mapping (Jira's status model will not match yours) declared as configuration, not code | Provider card plus a field-mapping table (`existing` Select per field) |
| 71 | Linear | `F12` spec only | 📄 | Driver over 68; Linear's GraphQL + webhook model is the one that will push the interface toward **push-based sync**, so design for webhooks from the start | Provider card |
| 72 | Sentry | `F12` spec only | 📄 | Different shape: not an issue source but an **event source that creates tasks from errors**. Model it as a task-creation trigger so it shares 98's dispatcher rather than 68's importer | Provider card with a trigger-rule row (severity, project, target workflow) |
| 73 | Slack | — | ❌ | Not a feature — **a channel behind 98's dispatcher**. Building it standalone means two notification code paths | A channel row inside notification settings |
...
**Where GateControl is at parity or better.** The single-task loop is complete and tested
...
loop is missing: 20 of its 21 agents, three of its four executors, all six integrations, the
...
|---|---|---|
| **Agent catalog replaces the agent enum** (19) | 20, 21, 26, 27, 60 | Every agent added before the catalog exists is a hardcoded special case — including the billing strip, which is the product's headline differentiator |
