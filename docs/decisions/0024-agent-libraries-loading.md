# 0024 — One library, three hand-overs: how each harness runtime is given MCP servers and Skills
...
[0023](./0023-docker-executor-cli.md) · **Enables:** [F24](../features/F24-harness-libraries.md)

## Context

[F24](../features/F24-harness-libraries.md) keeps MCP servers and Skills in one place and loads them
into every harness SoloW runs. SoloW drives three runtimes ([0003](./0003-agent-connection-protocol.md)),
...
**The library stores what an item is; a per-runtime materializer decides how the harness is told.**
`materializeLibraries` (`apps/orchestrator/src/harness/libraries.ts`) takes the resolved libraries,
...
1. **Claude Code** — `mcp.json` and a generated plugin (`.claude-plugin/plugin.json` +
   ...
   the Docker driver, and it is outside the worktree the harness creates — nothing reaches the diff.
...
**Resolution happens inside the durable step that spawns the harness, and is never memoized.**
`loadHarnessLibrariesForRun` decrypts the Secrets a server references; the same rule as the Harness
Profile's credential ([Principle IV](../../.specify/memory/constitution.md)). A Secret that is gone fails
the round by name — a server started without its token fails later, inside the harness, with a
...
- A directory-backed Skill is copied at launch. An edit to it after the harness started is the
...
- The ACP Skills directory is inside the worktree. It is excluded from git, so a commit made by
  SoloW never carries it; a harness that runs `git add -f` could still add it, which is the same
  trust the worktree already extends to the harness.
- A pass-through CLI harness gets no libraries. Making one take them is a runtime question, not a
...
- **Writing `.mcp.json` / `.claude/skills` into the repository checkout.** Claude Code would ask
  for trust on project-scoped servers, and every file would be in the harness's diff. Rejected.
