# 0003 — Integrate harnesses via the Agent Client Protocol (ACP)

**Status:** Accepted · **Date:** 2026-08-17 · **Deciders:** Architecture

> **2026-08-20:** Implemented in `packages/acp` and `apps/orchestrator/src/harness/acp-runner.ts`
...
> the harness's stdio. See **Implementation status** below for what landed, what stayed a vendor
> protocol and why, and what is still open.

## Context

SoloW must drive many different AI coding harnesses (Claude Code, Codex, Gemini CLI,
...
itself uses, and a large and growing set of harnesses support it.

## Decision

Integrate all harnesses through **ACP**, a single open standard, as the uniform boundary between
SoloW and harness tools.

## Considered options

- **Bespoke per-harness integrations** — Rejected: high cost, brittle, does not scale to many
  harnesses.
- **A build-your-own-harness framework** (in-process reasoning loop) — Rejected: the wrong
  layer. SoloW orchestrates *existing external harness tools*; it does not build a
  harness's reasoning loop, so frameworks for that solve a different problem.
- **ACP standard boundary (chosen)** — one interface to many harnesses; adding a harness is
  configuration, not engineering.

## Consequences

- Positive: broad harness support through one mechanism; orchestration is independent of any
  specific harness tool; new harnesses are added by configuration.
- Positive: choosing to drive external CLIs (rather than an in-process loop) is what makes
  subscription billing possible — the harness inherits the CLI's authentication
  (see [0005](./0005-subscription-authentication.md)).
- Negative: dependent on the maturity and evolution of the standard; harnesses differ in the
  capabilities they support (risk R-2), handled per Harness Profile.

## Implementation status

Landed with issue #58 (2026-08-20). Per acceptance criterion:

- **AC-1 — drive harnesses over ACP JSON-RPC on stdio.** `packages/acp` implements the client:
...
- **AC-2 — negotiate, do not assume (risk R-2).** Every optional capability reads `false` when
  the harness said nothing about it. `session/load` and `session/set_mode` are only ever sent when
  the harness advertised them, a prompt carrying an unadvertised content-block type is refused
  ...
  incoming request with `-32601`. A harness edits its own worktree with its own tools; proxying a
  filesystem through the orchestrator would widen the blast radius past the worktree for no gain.
- **AC-3 — Claude Code as one adapter among N.** `harness/runners.ts#createHarnessRunner` is the one
...
- **AC-4 — permissions surfaced, not granted.** `session/request_permission` becomes a
  ...
  `HarnessRegistry.respondPermission`, keyed by Workspace exactly like input and stop (Principle
  V). Its request id carries a per-run tag, because the harness's own JSON-RPC ids restart at 1 in
...
- **AC-6 — clean cancellation.** `stop()` sends the `session/cancel` notification, waits a
  ...
  and the ladder itself cannot reject the run's `outcome`: a harness that installs a `SIGTERM`
...
invokes a live harness (Principle VI). `FakeHarnessRunner` is retained and still drives the
lifecycle tests.

### Why `claude_code_stream_json` survives as a peer adapter

ACP is the uniform boundary **at the `HarnessRunner` interface**, not a requirement that every
harness be reached through an ACP process. Claude Code's own stream-JSON mode stays a first-class
...
SoloW provisions it — which `harnessCreatesOwnWorktree` answers in one line.
...
- **Row 26 — models and modes from live data.** The handshake and `session/new` already carry
  the harness's advertised modes, and `session/set_mode` is wired and guarded; writing what was
  negotiated back to `agent_catalog.capabilities` is not done.
- **Row 76 — passthrough MCP.** `session/new` is sent with `mcpServers: []`. A harness driven by
...
- **Authentication.** `initialize` negotiates `authMethods` and `NegotiatedCapabilities` carries
  ...
  criteria — the harnesses this build drives authenticate through the environment the billing guard
...
- **Turn model.** ACP v1 offers no way to type into a running turn, so operator input is queued
  as the next `session/prompt`. `HarnessHandle.send` already means "accepted", not "delivered now",
  ...
  turn 1 — input typed while the harness process was still starting used to take that place, and
  the harness's first prompt was a steering message with no task in it.
- **Durability of a pending permission.** The inbox lives in orchestrator memory, so a restart
  ...
  harness handle (`harness/registry.ts` says so); making a permission prompt durable is separate work.
...
This decision's claim was that a second harness should be *configuration* rather than engineering.
opencode is the first harness to test that claim, and it holds: `opencode acp` is an Agent Client
Protocol server negotiating protocol version 1 — the version `@solow/acp` implements — so the
integration is a single `agent_catalog` row. No package, no runner, no `HarnessProtocol` member, and
...
- **No `tool_result`.** ACP reports a tool's progress as `tool_call_update`, which the runner maps
  ...
  the way a stream-json harness's does — the transcript shows that a tool ran and finished, not what
  it returned.
...
orchestrator; the explanatory hints and a hand-written `<SelectItem>` list in the Harness Profile
...
They are now one `Record<HarnessProtocol, HarnessProtocolDescriptor>` in the contracts, which the
...
meant "adding a harness is a data row" was true only for harnesses already speaking one of the other
...
`text` column with no CHECK constraint. The type says `HarnessProtocol`; the database does not. A row
...
`harnessProtocolDescriptor`, which answers an undriven descriptor for anything unrecognised, and the
