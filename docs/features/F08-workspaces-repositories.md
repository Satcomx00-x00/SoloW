...
harnesses can work in parallel without colliding. A single Task can span multiple
...
- As a Solo Power User, I want each harness to work in isolation, so concurrent Tasks never
...
- As a Solo Power User, I want the local configuration a repository needs — its `.env`, say —
  to be present in every new Worktree, so the harness can run the test suite and verify its own
...
- **FR-8** A Repository carries an allowlist of file patterns — its **setup files** — copied
  from the Repository into each new Worktree before the Harness works in it. The allowlist is
...
- **The Harness runs in exactly one working directory: the primary attachment's Worktree.** Every
  other attachment is provisioned as a sibling Worktree and its absolute path is named to the
  Harness in the brief. That is the only way a Harness can reach a Repository it was not started
  in; until per-repository integration lands, a Harness that ignores the brief's `# Repositories`
...
- **Who creates the primary Worktree depends on what the attachment asks for.** A Harness that
  makes its own (Claude Code's `--worktree`) is left to, because that is what lets several Tasks
  share one Repository — but such a Harness branches from HEAD and names the branch itself. So an
  attachment that names a base ref, or a checkout branch other than the one SoloW derives,
  is provisioned by SoloW and the Harness is started inside it. Otherwise the Owner's base
...
- The brief names each Worktree's branch only once something can say what it is. A Worktree
  SoloW provisioned is on the attachment's branch by construction; one the Harness made for
...
- If any attached Repository is unreachable at Task start, the Task fails before running the
  Harness, with a reason naming that Repository. Only the Repository's name is recorded — the
...
- If a setup-file pattern matches nothing — or a matched file cannot be copied — SoloW
  ...
  files should still run, with less for the Harness to go on.
