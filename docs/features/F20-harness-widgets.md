...
A harness's only output channel is prose. Anything richer than a sentence — a question with fixed
...
operator can answer by tapping. SoloW defines one harness-agnostic vocabulary
...
than a change to the stream, the transcript or the harness.

The set is deliberately the same one the Claude app already draws — elicitation, visualizer,
cards, domain widgets, meta/install — so a harness that knows how to ask for a `step_card`
...
- As an Operator, I want a harness's question to arrive as buttons, so answering it is a tap
  rather than a sentence I have to phrase precisely enough for the model to parse.
- As an Operator, I want to see the plan a harness is following, so "how far along is this" is not
...
- **FR-1** A harness can emit a widget without any protocol support, as a fenced block in its own
...
- **FR-2** Every producer funnels through `parseWidget`, which is total: a malformed payload, an
  ...
  reason. A harness's emission is never silently dropped.
...
- **FR-5** An answer is validated against the widget that asked: only options the harness itself
...
- **FR-7** The feature is behind `ff-agent-widgets`, which governs both halves at once: the brief
  that teaches the harness the fence, and the scanner that reads it. A Workspace with the flag off
...
- Each `step_card` emission is a new row. The log is append-only, and a plan that rewrote its own
  history would destroy the evidence of what the harness believed earlier.
...
- **Blocking the harness on an answer.** A fenced widget is prose, so the answer reaches the harness
  as steering rather than as a tool result. Issue #75's task-scoped MCP surface is what turns a
  widget into a call the harness waits on; the contract and the renderers are already shaped for it,
