...
import { decryptForHarnessRun } from "./secret-store.js";

/**
 * What one harness run loads from the libraries (spec F24): every enabled item, plus the ones its
 ...
 * step that spawns the harness and its result must never be memoized — the same rule as the
 * Harness Profile's credential (`loadHarnessProbeContext`), and for the same Principle IV reason.
 *
 ...
 * the harness, after the run has already been paid for.
 */
export async function loadHarnessLibrariesForRun(
  db: Db,
  workspaceId: string,
  stepId: string | null,
): Promise<Result<HarnessLibraries, HarnessLibraryErrorCode>> {
  ...
  // Enabled everywhere, or named by this Step. Sorted by name so the harness sees one order.
  ...
  const resolve = (value: McpConfigValue): string | null => {
    ...
    return ciphertext === undefined
      ? null
      : `${value.prefix ?? ""}${decryptForHarnessRun(ciphertext)}`;
  };
  ...
  for (const row of chosenServers) {
    if (row.transport.kind === "stdio") {
      ...
      for (const [name, value] of Object.entries(row.transport.env)) {
        ...
        if (resolved === null) return err(HarnessLibraryErrorCode.SecretMissing);
        env[name] = resolved;
      }
      mcpServers.push({
        name: row.name,
