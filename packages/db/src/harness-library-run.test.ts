...
let loadHarnessLibrariesForRun: typeof import("./harness-library-run.js").loadHarnessLibrariesForRun;
beforeAll(async () => {
  process.env.SOLOW_SECRET_KEY = Buffer.alloc(32, 7).toString("base64");
  ({ encryptSecret } = await import("./secret-store.js"));
  ({ loadHarnessLibrariesForRun } = await import("./harness-library-run.js"));
});
 ...
 */
describe("loadHarnessLibrariesForRun", () => {
  let db: TestDb;
  let wsId: string;
  ...
  it("loads what is enabled plus what the step names, sorted by name, with secrets decrypted", async () => {
    ...
    // A Step needs a real Harness Profile behind it; the token above doubles as its credential.
    ...
    const loaded = await loadHarnessLibrariesForRun(db, wsId, step.id);
    ...
    const bare = await loadHarnessLibrariesForRun(db, wsId, null);
  ...
  it("refuses by name when a referenced secret is gone, or belongs to another workspace", async () => {
    ...
    const loaded = await loadHarnessLibrariesForRun(db, wsId, null);
    expect(loaded.ok ? null : loaded.error).toBe(HarnessLibraryErrorCode.SecretMissing);
  });
  ...
  it("writes a Secret's prefix in front of the decrypted value — `Bearer ` for a remote endpoint", async () => {
    ...
    const out = await loadHarnessLibrariesForRun(db, wsId, null);
