...
import { harnessCatalog, workspace } from "./schema.js";
 ...
 */
describe("ensureDefaultHarnessCatalog", () => {
  let db: TestDb;
  ...
  const rows = () =>
    db.select().from(harnessCatalog).where(eq(harnessCatalog.workspaceId, workspaceId));

  it("seeds every default, and answers with the Claude Code row's id", async () => {
    const id = await ensureDefaultHarnessCatalog(db, workspaceId);

    const seeded = await rows();
    expect(seeded.map((r) => r.key).sort()).toEqual(["claude_code", "opencode"]);
    // The id is what a first Harness Profile is pointed at, so it has to be the primary row's.
  ...
  it("is idempotent — a second call inserts nothing and returns the same id", async () => {
    const first = await ensureDefaultHarnessCatalog(db, workspaceId);
    const second = await ensureDefaultHarnessCatalog(db, workspaceId);

    expect(second).toBe(first);
    expect(await rows()).toHaveLength(DEFAULT_HARNESS_CATALOG.length);
  });
  ...
  it("backfills a default a Workspace predates, rather than skipping it", async () => {
    ...
    // so a "seed only when empty" check would leave it without the new harness forever.
    await ensureDefaultHarnessCatalog(db, workspaceId);
    await db.delete(harnessCatalog).where(eq(harnessCatalog.key, "opencode"));
  ...
  it("keeps each Workspace's catalog to itself (Principle V)", async () => {
    ...
    expect(
      await db
        .select()
        .from(harnessCatalog)
        .where(eq(harnessCatalog.workspaceId, other?.id ?? "")),
    ).toHaveLength(0);
  });
});
