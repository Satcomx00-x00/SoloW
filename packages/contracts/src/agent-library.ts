...
/**
 * The agent libraries: MCP servers and Skills, kept in one place and loaded into every agent
 * that runs (issue: agent libraries; spec F24).
 *
 * Two libraries, one shape of decision. An item is either *enabled* — loaded into every agent
 ...
 *
 * How an item reaches the agent is the *runtime's* concern, not the library's: Claude Code takes
...
/**
 * A name that doubles as an identifier on the agent's side — the key of an MCP server in a
 * config file, the directory of a Skill. Lower-case, so the same name means the same thing on
 * every filesystem the agent may run on.
 */
 ...
 * returns; the value behind it is decrypted only by the run loop, at the moment the agent is
...
/**
 * How the agent reaches the server: a process it spawns and talks to over stdio, or an HTTP
 ...
 *
 * `inline` is a Skill written in SoloW — the body *is* the `SKILL.md` the agent will read, and
...
export const AgentLibraryErrorCode = {
  /** Two items with one name would be one key on the agent's side — a coin flip. */
...
export type AgentLibraryErrorCode =
  (typeof AgentLibraryErrorCode)[keyof typeof AgentLibraryErrorCode];
...
export const mcpServerDto = z
  .object({
    ...
    /** Loaded into every agent run in the Workspace. Off: loaded only where a Step names it. */
    enabled: z.boolean(),
  })
...
/**
 * An MCP server as the agent will be started with it: every value resolved. Built by the run
 * loop from the library and the vault, inside the durable step that spawns the agent, and never
...
/** What one agent run loads: the enabled items, plus whatever its Step named. */
