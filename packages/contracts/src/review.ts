 ...
 * theory that feedback is the whole mechanism: it is what the orchestrator forwards to the harness
 * as `pendingFeedback` on the next round, and without it the harness resumes on exactly the same
 ...
 * to the harness, and callers with nothing to say are no longer blocked from saying so.
 */
export const reviewDecisionInput = z.object({
  sessionId: idSchema,
  decision: reviewDecisionSchema,
  /** Forwarded to the harness on the next round when present; never required. */
