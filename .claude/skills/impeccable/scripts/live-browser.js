...
(function () {
  ...
  const HOST = window.__IMPECCABLE_HOST__ || 'localhost';
  ...
  async function applyEditing() {
    ...
    try {
      ...
      const res = await fetch('http://' + HOST + ':' + PORT + '/manual-edit-stash?token=' + encodeURIComponent(TOKEN), {
        method: 'POST',
  ...
  async function fetchPendingCount() {
    try {
      const res = await fetch(
        'http://' + HOST + ':' + PORT + '/manual-edit-stash?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname),
      );
      if (!res.ok) return;
  ...
  async function onPendingPillClick() {
    ...
    try {
      const res = await fetch(
        'http://' + HOST + ':' + PORT + '/manual-edit-commit?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname) + '&async=1',
  ...
  async function onPendingTrashClick() {
    ...
    try {
      const res = await fetch(
        'http://' + HOST + ':' + PORT + '/manual-edit-discard?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname),
        { method: 'POST' },
      );
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      const res = await fetch(
        'http://' + HOST + ':' + PORT + '/manual-edit-commit?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname) + '&async=1&repair=1',
        { method: 'POST', keepalive: true },
      );
      if (!res.ok) throw new Error('HTTP ' + res.status);
      if (pendingKeepFixingBtn) pendingKeepFixingBtn.style.display = 'none';
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      const res = await fetch(
        'http://' + HOST + ':' + PORT + '/manual-edit-repair-decision?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function loadSvelteComponentParams(manifest) {
    ...
    const url = 'http://' + HOST + ':' + PORT + '/source?token=' + TOKEN + '&path=' + encodeURIComponent(paramsPath);
    try {
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    const url = 'http://' + HOST + ':' + PORT + '/source?token=' + TOKEN + '&path=' + encodeURIComponent(manifestPath);
    try {
      const res = await fetch(url);
  ...
  function probeJsxWrapperForOrphan(filePath, sessionId, opts) {
    ...
    const url = 'http://' + HOST + ':' + PORT + '/source?token=' + TOKEN + '&path=' + encodeURIComponent(filePath);
  ...
  function connectSSE() {
    evtSource = new EventSource('http://' + HOST + ':' + PORT + '/events?token=' + TOKEN);
  ...
  function sendEvent(msg, opts) {
    ...
    // authorizes the CORS preflight when the page runs on a non-loopback
    // dev host (ddev, Valet), since the preflight carries no request body.
    const doSend = () => fetch('http://' + HOST + ':' + PORT + '/events?token=' + encodeURIComponent(TOKEN), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  function loadModernScreenshot() {
    ...
    msLoadPromise = new Promise((resolve, reject) => {
      ...
      s.src = 'http://' + HOST + ':' + PORT + '/modern-screenshot.js';
    ...
    if (blob && hasAnnotations) {
      try {
        const uploadRes = await fetch(
          'http://' + HOST + ':' + PORT + '/annotation?token=' + encodeURIComponent(TOKEN) +
...
void main() {
  ...
  function fetchAgentPollingStatus() {
    fetch('http://' + HOST + ':' + PORT + '/status?token=' + TOKEN, { cache: 'no-store' })
  ...
  function loadDetectScript() {
    ...
    const s = document.createElement('script');
    s.src = 'http://' + HOST + ':' + PORT + '/detect.js';
  ...
  async function fetchDesignSystem() {
    ...
    try {
      const [jsonRes, rawRes] = await Promise.all([
        fetch(`http://${HOST}:${PORT}/design-system.json?token=${TOKEN}`, { cache: 'no-store' }),
        fetch(`http://${HOST}:${PORT}/design-system/raw?token=${TOKEN}`, { cache: 'no-store' }),
      ]);
