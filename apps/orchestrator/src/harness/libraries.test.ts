...
import type { HarnessLibraries } from "@solow/contracts";
import {
  acpMcpServers,
  claudeMcpConfig,
  gitInfoExcludePath,
 ...
 * and the protocol objects — checked on disk, because a path the harness cannot read is the whole
 * failure mode.
 */

const LIBRARIES: HarnessLibraries = {
  mcpServers: [
    {
      name: "github",
      transport: {
...
describe("materializeLibraries", () => {
  ...
  it("gives Claude Code absolute paths even for a relative library dir, since the harness's cwd is the repository", async () => {
