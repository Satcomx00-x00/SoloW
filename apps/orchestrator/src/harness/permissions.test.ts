...
describe("headlessFallbackPolicy", () => {
  it("refuses by default, even when the harness offered an allow", () => {
  ...
  it("declines when the harness offered nothing but refusals", () => {
    expect(
  ...
  it("declines when the harness offered no options at all", () => {
...
describe("PermissionInbox", () => {
  ...
  it("refuses an option the harness never offered, and says that is why", async () => {
    // The UI renders only the harness's own options; an id from anywhere else is a bug or an
  ...
  it("tells a stale answer apart from an unoffered option", async () => {
    ...
    // match anything on the table. Reporting either as "no harness is running" was the old bug.
    const inbox = new PermissionInbox(10_000);
