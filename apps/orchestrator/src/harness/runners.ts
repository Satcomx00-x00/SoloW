 ...
 * `protocol` column into something that can drive a harness.
 *
 * This is where "adding a harness is configuration, not engineering" (Decision 0003) becomes
 * true in code: a new harness whose protocol already has a driver is a database row, and a new
...
export interface HarnessRunnerDeps {
  /** Where the harness process runs — issue #1's `Executor`. */
  executor: Executor;
  /**
   * How much the harness may do without asking, from its Harness Profile (spec F05).
   *
   ...
   *   at once with the narrowest allow the harness offered". The request and its resolution are
  ...
  /**
   * The model and mode the Profile pinned, or absent for "whatever the harness chooses"
   * (issue #94).
   *
...
export function createHarnessRunner(
  ...
  switch (protocol) {
     ...
     * `HARNESS_PROTOCOLS.cli_passthrough` states and `unsupportedLaunchSettings` reports on.
     *
     * This case returning a runner rather than null is what makes "any harness, as a data row"
     * true for a harness that speaks neither protocol — the long tail the catalog is data for.
     */
...
export function unsupportedLaunchSettings(
   ...
   * form that accepts a pin the runner then ignores. `HARNESS_PROTOCOL_PINS` carries the reasoning
   ...
   */
  const pins = harnessProtocolDescriptor(protocol).pins;
