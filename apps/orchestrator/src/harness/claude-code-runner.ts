 ...
 * stays a peer behind the same `HarnessRunner` interface rather than being routed through ACP,
 ...
 *
 * There is nobody watching to answer a per-tool prompt, and the harness is confined to a
...
export interface ClaudeCodeRunnerOptions {
  ...
  /** The model the Harness Profile pinned, or absent to let the CLI choose (issue #94). */
  model?: string;
  ...
  start(opts: HarnessStartOpts): HarnessHandle {
    ...
    return {
      ...
      async send(text: string) {
        ...
        // transcript recorded only the harness's side of a conversation that visibly has two
        // parties (reported directly: an Owner sends a message and sees nothing acknowledge it,
        // then the harness's next reply appears to run straight into the previous paragraph with no
        // way to tell a new turn started). Echoed only once truly accepted, matching what
        // `FakeHarnessRunner` already simulates for every test that asserts this row exists.
...
export function toStreamEvent(update: ClaudeUpdate): HarnessStreamEvent | null {
  switch (update.kind) {
    case "tool_use":
      return {
        kind: "tool_use",
 ...
 * showed the harness saying everything a second time. That is not a rendering artefact to paper
 ...
 */
export function createStreamMapper(): (update: ClaudeUpdate) => HarnessStreamEvent | null {
