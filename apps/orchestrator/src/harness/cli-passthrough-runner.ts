...
/**
 * Any CLI at all, as a harness (issue #21).
 *
 * The third protocol was named in the enum long before it had a driver, and until now a Task
 * pointed at it was failed before it started. That made "adding a harness is a data row" true
 * only for harnesses that already speak one of two protocols — which is most of them, but not the
 ...
 *
 * As the last argument, not on stdin: a CLI harness invoked as `harness "do the thing"` is the
 ...
 */
function commandLine(opts: HarnessStartOpts): string[] {
...
export class CliPassthroughRunner implements HarnessRunner {
  ...
  start(opts: HarnessStartOpts): HarnessHandle {
     ...
     * interleaved them with the harness's answer would be unreadable.
     */
    let stderrTail = "";
    ...
    const emit = (channel: HarnessTextChannel, text: string) => {
      if (answered) return;
    ...
    const outcome: Promise<HarnessOutcome> = (async () => {
