 ...
 *
 * The ACP session asks; the operator answers through the WebSocket hub and the harness registry.
 ...
 * harness labelled `allow_once` grants whatever the harness happened to be asking for. So an
 ...
 * the harness gets the same answer it would get from an operator who said no. What it costs is
 ...
 *
 * `refuse` declines without choosing any of the harness's options — the same shape as an operator
 * closing the question, and the one answer that cannot grant something nobody consented to.
 *
 * `allow_once` is the opt-in posture, and even then it takes the *narrowest* allow the harness
 ...
 * a blanket one is listed first. A harness that offered no allow at all gets a refusal.
 */
...
export class PermissionInbox {
   ...
   * reading it — and `option_not_offered` means the click did not match anything the harness put
   * on the table. Collapsing them into one "no" was the old behaviour, and it left the terminal
   * telling an operator that the harness was not running while it was mid-turn.
   */
  answer(requestId: string, optionId: string): PermissionAnswer {
    const entry = this.pending.get(requestId);
    if (!entry) return "not_pending";
    // Only an option the harness actually offered: SoloW never invents one, for the same
