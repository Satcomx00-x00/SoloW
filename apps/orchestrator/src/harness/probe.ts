...
/**
 * Does this harness actually work? (2026-08-28.)
 *
 * Until now the answer arrived mid-run: a Harness Profile could name a command that was never
 * installed, or misspelled, and the first thing that noticed was a Task failing after it had
 * been queued, worktreed and briefed. Configuring a harness and finding out whether the
 ...
 * what the harness advertises — and no further: `initialize` and `session/new`, never a prompt.
...
export interface HarnessProbeInput {
  command: string;
  args: string[];
...
function reasonFor(cause: unknown, command: string): string {
  ...
  // The overwhelmingly common one, and the one worth naming precisely: the harness is not there.
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => {
          onTimeout();
          reject(new Error(`the harness did not answer within ${Math.round(ms / 1000)}s`));
        }, ms);
      }),
    ]);
  } finally {
...
/**
 * Start the harness, ask it what it is, and stop it.
 *
...
export async function probeHarness(
  ...
  try {
    if (input.protocol !== "acp") {
       ...
       * the harness for real. Killed immediately: this is a liveness check, not a run.
       */
    ...
    const peer = new JsonRpcPeer({
      ...
      // A probe asks and listens for its answers; a harness's notifications are not its business.
      onNotify: () => {},
    });
