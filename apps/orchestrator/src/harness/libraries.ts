...
/**
 * How each harness runtime is told about the libraries (spec F24, Decision 0024).
 *
 * The library says *what* an MCP server or a Skill is; this module says *how* a given harness is
 ...
 *    harness creates for itself (`--worktree`), which SoloW cannot pre-populate — so nothing is
 *    written into the harness's checkout and nothing shows up in its diff.
 *  - **ACP agents** (opencode, Claude Code over ACP) take servers on `session/new.mcpServers`,
 *    the protocol's own channel. Skills have no channel: they are files the harness discovers in
...
export interface MaterializedLibraries {
  /** Extra arguments for the harness's command line. */
  extraArgs: string[];
...
export async function materializeLibraries(opts: {
  libraries: HarnessLibraries;
  protocol: HarnessProtocol;
  catalogKey: string;
  /** Where generated config goes — a directory of the Task's own, mounted on every driver. */
  libraryDir: string;
  /** The worktree an ACP agent runs in, or null when the harness makes its own. */
  ...
  if (opts.protocol === "acp") {
    ...
    if (skills.length > 0) {
      if (opts.worktreePath) {
        ...
        notices.push(
          `Skills were not loaded: this harness runs without a worktree SoloW provisioned, so there is nowhere it would read them from.`,
        );
      }
    }
  ...
  return {
    ...NOTHING,
    notices: [
      `MCP servers and Skills were not loaded: SoloW does not know how to hand them to a "${opts.protocol}" harness.`,
    ],
  };
}
