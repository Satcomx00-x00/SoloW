...
/**
 * Pull widget emissions out of a harness's prose (see `@solow/contracts/widget.ts`).
 *
 * A harness's only output channel is text, so a widget arrives as a fenced block:
 *
 * ```solow:widget
 * {"kind":"ask_user_input","prompt":"Which database?","options":[…]}
 * ```
 *
 * This is deliberately the *lowest* rung of the ladder — it needs nothing from the harness's
 * protocol, so it works with Claude Code, with any ACP agent, and with a harness that has no tool
 ...
 *
 * **A block that never closes.** A harness can open a fence and then crash, or simply keep
...
/**
 * Stateful across a run's text: one scanner per harness stream. Not shared between channels —
...
/**
 * The instruction a harness needs in order to use any of this, appended to a Task's brief.
 *
 * Kept next to the parser on purpose: the day the fence changes, the sentence that teaches it
 * has to change in the same commit, or every harness keeps emitting the old one.
 */
export const WIDGET_BRIEF_INSTRUCTIONS = [
  ...
  "decision (a choice you made that shapes the product — see below), task_complete (how your",
  "run ended — see below).",
  "Emit a widget when it beats prose — a question with fixed answers, a diagram, a plan.",
  "",
  "A choice that shapes the product — a semantics you had to pick, a scope you narrowed, a",
  "criterion you read one way when it could be read another — is a `decision` widget, never a",
  "paragraph: state the question, the options you weighed with `why` each, the one you took",
  "(`chosen`) and your `reason`. The reviewer can pick another before the next step runs.",
  "",
  "```solow:widget",
  '{"kind":"decision","id":"include-semantics","question":"What does an include override admit?",',
  ' "options":[{"id":"gate","label":"Bypass the gate over nominated articles","why":"keeps the language/geo filter"},',
  '  {"id":"all","label":"Every article from the source","why":"needs the filter re-expressed as SQL"}],',
  ' "chosen":"gate","reason":"the filter lives in Python; re-expressing it is a second source of truth"}',
  "```",
  "",
  ...
  "   If the brief asked you a decision under *Decision to make*, answer it in a `decision` field",
  '   ("yes" or "no") on this same widget.',
  "",
  "```solow:widget",
  '{"kind":"task_complete","outcome":"changes_ready","summary":"Pinned 5 dependencies","decision":"no"}',
  "```",
  "",
  ...
  "it does not approve, merge or close anything. A person opens the review. The `outcome` you",
  "report may decide which step of a workflow runs next — report the one that is true.",
].join("\n");
