...
/**
 * The Agent Client Protocol as SoloW's harness transport (Decision 0003 / issue #58).
 *
 * The mirror of `ClaudeCodeRunner`, and deliberately the same shape: bind the protocol driver to
 * `Executor.spawn`, map the protocol's updates onto `HarnessStreamEvent`, classify a failure from
 * what the harness said and then from its stderr. Two things differ, and both are the protocol's
 * doing rather than a design choice here:
 *
 * - **The worktree is SoloW's.** ACP has no `--worktree`; the lifecycle creates the
 *   directory and this runner is pointed at it. So `workspacePath` is known before the harness
 *   says anything, and is reported even when the run fails — the harness was isolated whether or
 ...
 *   queues the next `session/prompt`. `HarnessHandle.send`'s `Promise<boolean>` already means
...
export interface AcpRunnerOptions {
  /** Where the harness process actually runs — issue #1's `Executor`. */
  executor: Executor;
  /** Diagnostics sink for the harness's stderr. Never receives protocol traffic. */
  onStderr?: (text: string) => void;
  /**
   * The session mode the Harness Profile pinned (issue #94), or absent to leave the harness on its
   * own default. Only ever sent when the harness advertised it — `startAcpSession` checks the
...
export class AcpRunner implements HarnessRunner {
  ...
  start(opts: HarnessStartOpts): HarnessHandle {
      ...
      session = startAcpSession(
        {
          ...
          onPermission: async (request) => {
            ...
            // harness is about to give up on the turn, and a reviewer reading the transcript
    ...
    return {
      ...
      async stop() {
        stopRequested = true;
        // Release the turn first: a harness blocked on a permission nobody will now answer would
...
/**
 * Classify a failed run. What the harness said about the failure is the better signal when it
...
export function toStreamEvent(update: AcpUpdate): HarnessStreamEvent | null {
  switch (update.kind) {
    case "tool_call":
    ...
    case "text":
      ...
      // harness's reasoning, and hiding it would make the terminal disagree with the transcript.
      // A `user` chunk is the harness echoing back what it was told, which is worth seeing too.
