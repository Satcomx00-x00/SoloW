...
import {
  ...
  updateMcpServer,
  updateSkill,
} from "../dal/harness-library.js";
import { libraryProcedure, router, unwrap } from "../trpc.js";

/**
 * The harness libraries API (spec F24): `library.mcp.*` and `library.skill.*`.
 *
 ...
 * rows say is loaded into every harness, so a harness's token must not be able to write them.
 */
    ...
    create: libraryProcedure
      .meta({
        openapi: {
          ...
          protect: true,
          summary:
            "Add an MCP server — a stdio command or an HTTP endpoint — to the library. Each env variable or header is a literal or a reference to one of this Workspace's Secrets. `enabled` loads it into every harness run; off, it is loaded only by the Workflow Steps that name it.",
        },
      })
    ...
    create: libraryProcedure
      .meta({
        openapi: {
          ...
          protect: true,
          summary:
            "Add a Skill — written inline as the SKILL.md the harness will read, or pointing at a directory on the host that holds one. `enabled` loads it into every harness run; off, only the Workflow Steps that name it load it.",
        },
      })
