...
import { CommonErrorCode, HarnessLibraryErrorCode, WorkflowErrorCode } from "@solow/contracts";
import { ensureDefaultHarnessCatalog, workspace } from "@solow/db";
...
/**
 * The harness libraries (spec F24) against a real in-memory database: the two tables, the
...
describe("harness libraries", () => {
  ...
  describe("MCP servers", () => {
    ...
    it("switches a server on for every harness, and off again", async () => {
      const { c, secretId } = await fixture(db, "acme");
      const created = await c.library.mcp.create(github(secretId));
    ...
    it("refuses two servers with one name, and a rename onto a taken one", async () => {
      ...
      expect(await errMessage(() => c.library.mcp.create(github(secretId)))).toBe(
        HarnessLibraryErrorCode.NameTaken,
      );
    ...
    it("refuses a reference to another Workspace's secret, on create and on update", async () => {
      ...
      expect(await errMessage(() => acme.c.library.mcp.create(github(rival.secretId)))).toBe(
        HarnessLibraryErrorCode.NotInWorkspace,
      );
      ...
      ).toBe(HarnessLibraryErrorCode.NotInWorkspace);
  ...
  describe("Skills", () => {
    ...
    it("refuses a name that is not a slug — it becomes a directory on the harness's side", async () => {
      const { c } = await fixture(db, "acme");
      expect(
        await errMessage(() =>
  ...
  describe("Workflow Steps naming library items", () => {
    async function pipeline(c: ReturnType<typeof caller>, secretId: string) {
      const catalogId = await ensureDefaultHarnessCatalog(db, (await c.workspace.get({})).id);
  ...
  describe("bulk import", () => {
    ...
    it("unpacks a dropped zip and imports what it holds, from SoloW's own skills directory", async () => {
      ...
      ).toBe(HarnessLibraryErrorCode.ImportArchiveInvalid);
    });
    ...
    it("says when the source is not there, and when a repository cannot be cloned", async () => {
      ...
      ).toBe(HarnessLibraryErrorCode.ImportSourceNotFound);
      expect(
  ...
  it("keeps its writes off the external MCP surface, and is refused when the flag is off", async () => {
    // A token a harness holds must not be able to hand that harness a new server — but a Workflow
