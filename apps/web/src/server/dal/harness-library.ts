...
/**
 * The harness libraries (spec F24): MCP servers and Skills, one table each, one DAL.
 *
 ...
 * written, for the reason `addWorkflowStep` resolves a Harness Profile: a foreign key proves the
 ...
 * nothing more; decryption happens in `loadHarnessLibrariesForRun` (`@solow/db`), inside the run
 * loop, at the moment the harness starts (Principle IV).
 */

type NotFound = typeof CommonErrorCode.NotFound;
type LibraryError = NotFound | typeof CommonErrorCode.ValidationFailed | HarnessLibraryErrorCode;
...
function checkSecrets(
...
): Result<void, HarnessLibraryErrorCode> {
  ...
  const found = tx
    ...
    .all();
  return found.length === ids.length ? ok(undefined) : err(HarnessLibraryErrorCode.NotInWorkspace);
}
  ...
  return ctx.db.transaction(
    (tx): Result<McpServerDto, LibraryError> => {
      const [taken] = tx
        ...
        .limit(1)
        .all();
      if (taken) return err(HarnessLibraryErrorCode.NameTaken);
...
export async function deleteMcpServer(
  ctx: RequestContext,
  id: string,
): Promise<Result<void, NotFound | HarnessLibraryErrorCode>> {
  return ctx.db.transaction(
    (tx): Result<void, NotFound | HarnessLibraryErrorCode> => {
      const [row] = tx
        .select({ id: mcpServer.id })
        .from(mcpServer)
        ...
        .all();
      if (!row) return err(CommonErrorCode.NotFound);
      if (namedByAStep(tx, ctx, "mcp", id)) return err(HarnessLibraryErrorCode.InUse);
      tx.delete(mcpServer)
        .where(and(eq(mcpServer.workspaceId, ctx.workspaceId), eq(mcpServer.id, id)))
        .run();
      return ok(undefined);
 ...
 * what sits beside the SKILL.md — scripts, references, assets — is what the harness gets. Checked
  ...
  return ctx.db.transaction(
    (tx): Result<void, NotFound | HarnessLibraryErrorCode> => {
      const [row] = tx
        ...
        .all();
      if (!row) return err(CommonErrorCode.NotFound);
      if (namedByAStep(tx, ctx, "skill", id)) return err(HarnessLibraryErrorCode.InUse);
      tx.delete(skill)
        .where(and(eq(skill.workspaceId, ctx.workspaceId), eq(skill.id, id)))
        .run();
      return ok(undefined);
