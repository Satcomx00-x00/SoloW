...
import type { HarnessActivity } from "./transcript";

/**
 * What the harness is doing while it is not producing text.
 *
 ...
 *
 * So the foot of the transcript always answers the question. `harnessActivity` decides *what* is
 * happening from the rows themselves; this decides how to say it, and says nothing at all when
 * the run is over or when the harness is blocked on a question the operator can already see.
 */
...
/** The line under the transcript. Present whenever the harness is working, absent otherwise. */
export function HarnessActivityLine({ activity }: { activity: HarnessActivity }) {
  return (
    <p
 ...
 * that the harness has been in `Bash` for ninety seconds knows to go look at the command, and one
 * told only that something is in progress does not.
 */
function label(activity: HarnessActivity): string {
  switch (activity.kind) {
    case "launching":
      return "Launching the harness…";
    case "tool":
 ...
 *
 * The panel used to say "No harness output yet. Launch the task to start a run." for the whole
...
export function LaunchingPanel() {
  return (
    <div
      aria-live="polite"
      data-harness-activity="launching"
