...
/**
 * Harness Profile deletion from Settings. `agent_profile.id` is a real foreign key on Tasks,
...
describe("HarnessProfilesSection", () => {
  it("deletes a Profile only after the Owner confirms it", async () => {
    ...
    const trigger = await screen.findByRole("button", {
      name: "Delete the harness profile Claude Code (subscription)",
    });
  ...
  it("refuses to offer deletion for a Profile a Task still holds, and says what holds it", async () => {
    ...
    const trigger = await screen.findByRole("button", {
      name: "Delete the harness profile Busy profile",
    });
  ...
  it("offers deletion freely for a Profile nothing references", async () => {
    ...
    const trigger = await screen.findByRole("button", {
      name: "Delete the harness profile Unused profile",
    });
  ...
  it("surfaces a delete failure to the Owner instead of failing silently", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Delete the harness profile Raced profile" }),
    );
    const dialog = await screen.findByRole("alertdialog");
    fireEvent.click(within(dialog).getByRole("button", { name: "Delete profile" }));
...
/**
 * Extending the harness catalog (spec F05 AC-1, issue #10/#58). The seeded `claude_code` row
 * (protocol `claude_code_stream_json`) has no permission channel; `acp` does — this is the form
 * that lets an Owner name a harness on that protocol at all, since until it existed no Harness
 * Profile could ever be pointed at one.
 */
describe("HarnessProfilesSection — harness catalog", () => {
  it("is collapsed by default — most Owners never need it", async () => {
    ...
    await screen.findByText("Add a custom harness");
  ...
  it("declares a new catalog entry with the fields that matter for billing integrity", async () => {
    ...
    fireEvent.click(await screen.findByText("Add a custom harness"));
 ...
 * text rather than a dropdown on purpose — what a harness offers comes from its handshake, so
 ...
 */
describe("model and mode on a Harness Profile", () => {
  ...
  it("sends null for a pin left empty, so the harness keeps the choice", async () => {
    const { log } = renderWithTrpc(<HarnessProfilesSection />, {
      ...baseHandlers,
      "profile.agent.list": () => ({ items: [], nextCursor: null }),
...
/**
 * The pin fields suggest what the harness advertised, and a pin it no longer advertises is said
 * (issue #94, AC-2 / AC-3).
 *
...
describe("advertised capabilities in the Profile form", () => {
  ...
  it("offers the cached models and modes as suggestions", async () => {
    ...
    await screen.findByLabelText("Model");
    // The datalist is the suggestion surface; its options are what the harness last advertised.
    const modelOptions = [...document.querySelectorAll("#harness-model-options option")].map(
      (o) => (o as HTMLOptionElement).value,
    );
    const modeOptions = [...document.querySelectorAll("#harness-mode-options option")].map(
      (o) => (o as HTMLOptionElement).value,
    );
  ...
  it("marks a Profile whose pin the harness no longer advertises", async () => {
    renderWithTrpc(<HarnessProfilesSection />, {
      ...baseHandlers,
      "profile.agentCatalog.list": catalogWithCaps,
      "profile.agent.list": () => ({
...
/**
 * Managing more than one harness (2026-08-28). A Workspace ships with Claude Code and opencode,
 * and they differ on exactly the axis this form exposes: stream-JSON takes a model and has no
 * session mode, ACP expresses both through the ids the harness advertised, and a passthrough CLI
 * is handed the brief and nothing else. The form has to refuse the pin the chosen harness cannot
 ...
 */
describe("HarnessProfilesSection — two harnesses with different protocols", () => {
  const twoHarnesses = {
    ...baseHandlers,
    "profile.agentCatalog.list": () => [
  ...
  const chooseHarness = async (displayName: string) => {
    fireEvent.click(await screen.findByRole("combobox", { name: "Harness" }));
    fireEvent.click(await screen.findByRole("option", { name: displayName }));
  };

  it("disables Mode for the stream-JSON harness, which has no notion of one", async () => {
    renderWithTrpc(<HarnessProfilesSection />, twoHarnesses);
  ...
  it("offers both pins to the ACP harness, which can express either", async () => {
    renderWithTrpc(<HarnessProfilesSection />, twoHarnesses);
    await waitFor(() => expect(isDisabled("Model")).toBe(false));

    await chooseHarness("opencode");
    ...
    // custom-harness form below carries the same hints for its own protocol picker.
  ...
  it("never submits a pin the chosen harness's protocol would ignore", async () => {
 ...
 *
 * Nothing verified a Harness Profile until a run failed on it: a misspelled command, a harness
 ...
 */
describe("HarnessProfilesSection — testing a profile", () => {
  const oneProfile = {
    ...baseHandlers,
    "profile.agent.list": () => ({
      items: [
  ...
  it("shows the harness's own sign-in next to a green result", async () => {
  ...
  it("puts the reason on screen when the harness does not work", async () => {
    renderWithTrpc(<HarnessProfilesSection />, {
      ...oneProfile,
      "profile.agent.probe": () =>
        report({
