...
export function IntegrationsSection() {
  ...
  const connectedCount = integrations.data?.length ?? 0;
  ...
  return (
    <SettingsSection
      caption="The GitHub, GitLab and Gitea accounts this Workspace reads work from. Connect one with a personal access token, then import a repository to bring its Issues across."
      id="integrations"
      status={
        integrations.isSuccess ? (
          <SectionStatus tone={connectedCount === 0 ? "waiting" : "idle"}>
            {connectedCount === 0
              ? "None connected"
              : `${connectedCount} connected · ${repos.data?.items.length ?? 0} repositories`}
          </SectionStatus>
        ) : null
      }
      title="Integrations"
    >
      {integrations.isPending ? (
        ...
        <SettingsEmpty>
          No provider connected. Issues, branches and change requests are mirrored through one.
        </SettingsEmpty>
      ) : null}
      ...
      <SettingsCreate
        defaultOpen={integrations.isSuccess && connectedCount === 0}
